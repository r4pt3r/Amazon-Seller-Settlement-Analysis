...
if uploaded_file is not None:
    try:
        ...
        ORDER_SUMMARY = ORDER.groupby('order-id').agg({
            'amount': 'sum',
            'sku': 'first',
            'quantity-purchased': 'first'
        }).reset_index()
        ...
        st.write(f"📋 **{len(ORDER_SUMMARY)}** orders processed")
    ...
    try:
        ...
        if 'SKU' not in cogs_df.columns or 'COGS' not in cogs_df.columns:
            ...
            ORDER_SUMMARY_WITH_COGS['profit'] = (
                ORDER_SUMMARY_WITH_COGS['amount'] - 
                (ORDER_SUMMARY_WITH_COGS['COGS'] * ORDER_SUMMARY_WITH_COGS['quantity-purchased'])
            )
            ...
            with col1:
                total_revenue = ORDER_SUMMARY_WITH_COGS['amount'].sum()
                st.metric("💰 Total Revenue", f"₹{total_revenue:,.2f}")
            
            with col2:
                total_cogs = (ORDER_SUMMARY_WITH_COGS['COGS'] * ORDER_SUMMARY_WITH_COGS['quantity-purchased']).sum()
            ...
            with col1:
                ...
                top_orders = ORDER_SUMMARY_WITH_COGS.nlargest(5, 'profit')[['order-id', 'sku', 'amount', 'profit']]
                st.dataframe(top_orders)
            
            with col2:
                st.subheader("⚠️ Bottom 5 Profitable Orders")
                bottom_orders = ORDER_SUMMARY_WITH_COGS.nsmallest(5, 'profit')[['order-id', 'sku', 'amount', 'profit']]
